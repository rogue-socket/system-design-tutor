...
Runs `validate_progress.py` against the template and `fixtures/progress_valid.json`, plus structural checks: curriculum step IDs, exercise-bank coverage tags, SKILL.md frontmatter and references, incidents.md tier coverage, doc/validator sync, Tier 6 primary-sources + per-item citations, and the production-readiness template existing alongside `practical-mode.md` documenting it and `exercise-bank.md` declaring `production_readiness_focus` with at least 3 populated entries. All pass = the skill's mechanical scaffolding is intact.
